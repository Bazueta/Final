...
public class Retirement {
	...
	public double MonthlySavings() {
		...
		pmt = PMT(this.getdAnnualReturnWorking() / 12.0, this.getiYearsToWork() * 12.0, 
        		0.0, -TotalAmountToSave(), false);
	...
	public double TotalAmountToSave() {
		...
		double pv = 0;
		pv = PV(this.getdAnnualReturnRetired() / 12.0, this.getiYearsRetired() * 12.0,
				-(this.getdRequiredIncome() - this.getdMonthlySSI()), 0, false);
		//	Hint: Here's how to round a number: pv = Math.round(pv * 100.0) / 100.0;
		pv = Math.round(pv * 100) / 100.0;
		return pv;
	}
